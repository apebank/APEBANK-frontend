...
import { StakingContract, sAPETokenContract, APETokenContract } from "../../abi";
...
export const loadAppDetails = createAsyncThunk(
    ...
    async ({ networkID, provider }: ILoadAppDetails) => {
        ...
        const totalSupply = (await APEContract.totalSupply()) / Math.pow(10, 9);
        const circSupply = (await sAPEContract.circulatingSupply()) / Math.pow(10, 9);
        ...
        const timeBondsAmountsPromises = allBonds.map(bond => bond.getAPEAmount(networkID, provider));
        ...
        const circ = await sAPEContract.circulatingSupply();
        const stakingRebase = stakingReward / circ;
        const fiveDayRate = Math.pow(1 + stakingRebase, 5 * 3) - 1;
        const stakingAPY = Math.pow(1 + stakingRebase, 365 * 3) - 1;

        console.log({ stakingAPY, stakingReward, circ, stakingRebase });
