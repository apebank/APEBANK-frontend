...
import { USDTTokenContract, ZapinContract } from "../../abi";
...
export const changeApproval = createAsyncThunk("zapin/changeApproval", async ({ token, provider, address, networkID }: IChangeApproval, { dispatch }) => {
    ...
    const tokenContract = new ethers.Contract(tokenAddress, USDTTokenContract, signer);
    ...
    try {
        ...
        const pendingTxnType = "approve_" + tokenAddress;
        ...
        try {
            ...
            if (bond.isLP) {
                if (token.isETH) {
                    ...
                    zapinTx = await zapinContract.ZapInLp(
                        getTokenAddress(networkID, token.name),
                        bondAddress,
                        valueInWei,
                        minReturnAmount,
                        swapTarget,
