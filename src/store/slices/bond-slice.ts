...
export const calcBondDetails = createAsyncThunk("bonding/calcBondDetails", async ({ bond, value, provider, networkID }: ICalcBondDetails, { dispatch }) => {
    ...
    marketPrice = marketPrice * usdtPrice;
    ...
    if (!!value && bondQuote > maxBondPrice) {
        dispatch(error({ text: messages.try_bond_more(maxBondPrice.toFixed(2).toString()) }));
    }
    ...
    if (bond.isLP) {
        ...
        purchased = purchased / Math.pow(10, decimals);

        if (bond.name === boba.name) {
            const tokenPrice = getTokenPrice("BOBA");
