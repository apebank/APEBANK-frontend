...
import { wsAPETokenContract } from "../../abi";
...
export const changeApproval = createAsyncThunk("wrapping/changeApproval", async ({ provider, address, networkID }: IChangeApproval, { dispatch }) => {
    ...
    const memoContract = new ethers.Contract(addresses.sAPE_ADDRESS, wsAPETokenContract, signer);

    let approveTx;
    try {
        const gasPrice = await getGasPrice(provider);

        approveTx = await memoContract.approve(addresses.wsAPE_ADDRESS, ethers.constants.MaxUint256, { gasPrice });
    ...
    const wmemoAllowance = await memoContract.allowance(address, addresses.wsAPE_ADDRESS);
...
export const changeWrap = createAsyncThunk("wrapping/changeWrap", async ({ isWrap, value, provider, networkID, address }: IChangeWrap, { dispatch }) => {
    ...
    const wmemoContract = new ethers.Contract(addresses.wsAPE_ADDRESS, wsAPETokenContract, signer);
